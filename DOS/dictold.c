...
static void recurse(int pos, ushort n)
{
	...
	while (!abortMatch)
	{
		...
		if (pos==(patLen-1) || !patUseAll) {
			if (Nodes(n).isterminal) {
				if (!patAnagrams || !Nodes(n).matched)
				{
					newWord[pos+1]=0;
...
/* matchPattern builds every possible combination of the letters
	in pat, and calls recurse to find all matches for these. */

static void patRecurse(int pos)
{
	int i;
...
void matchPattern(char *pat, int anagrams, int allLengths)
{
	...
	patLen = j;
	patUseAll = !allLengths;
	patAnagrams = (anagrams>1);
	clearMarks();
	if (anagrams) patRecurse(0);
	else {
		for (i=0;i<j;i++)
			newPat[i] = (char)i;
		recurse(0,1);
